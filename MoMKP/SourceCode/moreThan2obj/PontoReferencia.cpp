...
class PontoReferencia {
	public:
		...
		vector<pair<Knapsack*, double> > membros; // membros (Knapsack e distancia)
	public:
	...
	public:
		...
		Knapsack * get_membro_mais_proximo() {
		...
		void add_member(Knapsack * sol, double distance) {
			membros.push_back(make_pair(sol, distance));
		}
		void remove_member(Knapsack * sol) {
			for (size_t i = 0; i < membros.size(); ++i) {
