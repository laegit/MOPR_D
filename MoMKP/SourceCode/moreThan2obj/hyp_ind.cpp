...
private:
	...
	int  obj[8];  /* obj[i] = 0 means objective i is to be minimized */
	int  method;  /* 0 = no reference set, 1 = with respect to reference set */
	double nadir[8];;  /* reference point for hypervolume calculation */
	double *curr_run;
...
public:
	...
	double getHypervolume(ParetoSet *current_position, std::vector<double> lbound, std::vector<double> ubound){
		...
		list<Knapsack *> sol = current_position->getElementos();
		list<Knapsack *>::iterator i = sol.begin();
		int cont = 0, k=0;
		while (i!=sol.end()){
			for (int objjjj = 0; objjjj < my_momkp->dimension_; ++objjjj){
				curr_run[k++] = 1.0+((double)(*i)->profit(objjjj)-lbound[objjjj])/(ubound[objjjj]-lbound[objjjj]);
			}

			// curr_run[k++] = 1.0+((double)(*i)->profit(0)-lbound[0])/(ubound[0]-lbound[0]);
			// curr_run[k++] = 1.0+((double)(*i)->profit(1)-lbound[1])/(ubound[1]-lbound[1]);
			// fprintf(stdout, "%.9e %.9e\n", 1.0+((double)(*i)->profit(0)-lbound[0])/(ubound[0]-lbound[0]), 1.0+((double)(*i)->profit(1)-lbound[1])/(ubound[1]-lbound[1]));
			cont++;
			i++;
		}
