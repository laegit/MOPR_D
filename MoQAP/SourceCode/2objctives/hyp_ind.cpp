...
public:
	...
	Hypervolume(){
		...
		for (int i=0; i<n_objetivos; i++){
			obj[i] = 0; // objetivo i deve ser minimizado
			nadir[i] = 2.1;
		} 
	...
	double getHypervolume(ParetoSet *current_position, std::vector<double> lbound, std::vector<double> ubound){
		...
		list<Solution *> sol = current_position->getElementos();
		list<Solution *>::iterator i = sol.begin();
		int cont = 0, k=0;
		while (i!=sol.end()){
			for (int objjjj = 0; objjjj < n_objetivos; ++objjjj){
				curr_run[k++] = 1.0+((double)(*i)->getObj(objjjj)-lbound[objjjj])/(ubound[objjjj]-lbound[objjjj]);
			}
