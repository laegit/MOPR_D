...
class Version5_D: public PathRelinking{
	private:
		...
		// bool add_solution_subespaco_cd(SolucaoEdgeSet *nova_solu, int subb){
		...
		// 		crownding_distance_assigment(subespacos[subb]);
		// 		subespacos[subb].sort(compare2); // ordena por CD decrescente
		// 		subespacos[subb].pop_back();
		// 	}
		...
		// 	SolucaoEdgeSet *t = new SolucaoEdgeSet(nova_solu->nEdges);
		...
		bool add_solution_subespaco(SolucaoEdgeSet *nova_solu, int subb){
			...
			while (i != subespacos[subb].end()) {
				...
				double distance_it = sqrt(((*i)->getObj(0)-ideal[subb][0])*((*i)->getObj(0)-ideal[subb][0]) + ((*i)->getObj(1)-ideal[subb][1])*((*i)->getObj(1)-ideal[subb][1]) );
			...
			if (remover.size() == 0 && subespacos[subb].size()+1 > MAX_SUB_POPULATION) {
				...
				double distance_nova_solu = sqrt((nova_solu->getObj(0)-ideal[subb][0])*(nova_solu->getObj(0)-ideal[subb][0]) + (nova_solu->getObj(1)-ideal[subb][1])*(nova_solu->getObj(1)-ideal[subb][1]) );
		...
		void initialize(BoundedParetoSet *approx_set){
			...
			for (int i = 0; i<NUMSUMPROBLEMAS; i++){
				...
				// double min_t = INT_MAX;
				// for (list<SolucaoEdgeSet *>::iterator it_2 = subespacos[i].begin(); it_2!=subespacos[i].end(); it_2++){
				// 	double norma_t = 0.0;
				// 	for (int obj=0; obj<NUMOBJETIVOS; obj++){
				// 		norma_t += (Z_min_sub[i][obj]-(*it_2)->getObj(obj))*(Z_min_sub[i][obj]-(*it_2)->getObj(obj));
				// 	}
				// 	double de_t = sqrt(norma_t);
				// 	if (de_t<min_t) min_t = de_t;
				// }
			...
			// // iniciliza o vetor ideal para cada subproblema
			// double t = calcula_t(approx_set); // t é um escalar
			// // cout<<"t = "<<t<<endl;
			// // cout<<"Z = "<<Z_min[0]<<" "<<Z_min[1]<<endl;
			// for (int i = 0; i<NUMSUMPROBLEMAS; i++){
			// 	for (int obj = 0; obj<NUMOBJETIVOS; obj++){
			// 		ideal[i][obj] = t*lambda[i][obj] + Z_min[obj];// t é um escalar
			// 	}
			// }
			...
			for (int cont = 0; cont<totalSai; cont++){
					...
					if (amostralSai[iii]!=-1){
						...
						for (int jd=0; jd<totalEnt; jd++){
							...
							if (aux->uf.sameClass(a,b)==false){
								...
								double distance = sqrt((new_objj_0-ideal[subproblemaATUAL][0])*(new_objj_0-ideal[subproblemaATUAL][0]) + (new_objj_1-ideal[subproblemaATUAL][1])*(new_objj_1-ideal[subproblemaATUAL][1]) );
				...
				if (sai != -1 && entra != -1){
					...
					for (int subk=0; subk<NUMSUMPROBLEMAS; subk++){
						...
						double norma_lambda = lambda[subk][0]*lambda[subk][0] + lambda[subk][1]*lambda[subk][1];
						double norma_aux = (aux->getObj(0)-Z_min[0])*(aux->getObj(0)-Z_min[0]) + (aux->getObj(1)-Z_min[1])*(aux->getObj(1)-Z_min[1]); 
						double produto_escalar = (lambda[subk][0]*(aux->getObj(0) - Z_min[0])) +  (lambda[subk][1]*(aux->getObj(1) - Z_min[1]));
...
public:
	...
	// // péssimo
	// BoundedParetoSet *MOPR_D_2(){
	// 	int contAval = 0;
	// 	std::vector<SolucaoEdgeSet *> sub_pop; // tamanho NUMSUMPROBLEMAS
	// 	for (int i=0; i<NUMSUMPROBLEMAS; i++){
	// 		sub_pop.push_back(new SolucaoEdgeSet(NUMEROVERTICES-1));
	// 	}

	// 	do{
	// 		for (int i=0; i<NUMSUMPROBLEMAS; i++){
	// 			crownding_distance_assigment(subespacos[i]);
	// 			int a_i = IRandom(0,subespacos[i].size()-1);
	// 			int b_i = IRandom(0,subespacos[i].size()-1);
	// 			list<SolucaoEdgeSet * >::iterator it_a = subespacos[i].begin();
	// 			for ( int ont = 0; ont<a_i; ont++) it_a++;
				
	// 			list<SolucaoEdgeSet * >::iterator it_b = subespacos[i].begin();
	// 			for ( int ont = 0; ont<b_i; ont++) it_b++;

	// 			if ((*it_a)->distance > (*it_b)->distance) *sub_pop[i] = *(*it_a);
	// 			else *sub_pop[i] = *(*it_b);
	// 		}

	// 		// cout<<"aqui"<<endl;
	// 		for (int origm=0; origm<NUMSUMPROBLEMAS; origm++){
	// 			for (int dest=0; dest<NUMSUMPROBLEMAS; dest++){
	// 				if (origm!=dest){
	// 					// cout<<"antes"<<endl;
	// 					PR(retorno, sub_pop[origm], sub_pop[dest], contAval);
	// 					// cout<<"depois"<<endl;
	// 				}
	// 			}
	// 		}
	...
	//depreciado
	// BoundedParetoSet *MOPR_D_4(int &contPares){
	// 	int contAval = 0;
	// 	SolucaoEdgeSet *origm_p = new SolucaoEdgeSet(NUMEROVERTICES-1);
	// 	SolucaoEdgeSet *dest_p = new SolucaoEdgeSet(NUMEROVERTICES-1);
	// 	do{
	...
	// 			crownding_distance_assigment(subespacos[sub_a]);
	// 			crownding_distance_assigment(subespacos[sub_b]);
	...
	// 			SolucaoEdgeSet *origm, *dest;
	// 			if ((*it_a1)->distance > (*it_a2)->distance)  origm = *it_a1;
	// 			else origm = *it_a2;

	// 			if ((*it_b1)->distance > (*it_b2)->distance)  dest = *it_b1;
	// 			else dest = *it_b2; 
	...
	Version5_D(BoundedParetoSet *approx_set, double lambda2[NUMSUMPROBLEMAS][NUMOBJETIVOS]){
		for (int i=0; i<NUMSUMPROBLEMAS; i++){
			for (int j=0; j<NUMOBJETIVOS; j++) lambda[i][j] = lambda2[i][j];
		}
		...
		// for (int i=0; i<NUMSUMPROBLEMAS; i++){
			...
			// cout<<"Subproblema "<<i<<" ("<< lambda[i][0] <<" "<<lambda[i][1]<< ")"<<endl; //( "<< bestSolution[i]->getObj(0)<<" "<< bestSolution[i]->getObj(1)<<"): "<<endl;
			
			// for (list<SolucaoEdgeSet *>::iterator it_1=subespacos[i].begin(); it_1!=subespacos[i].end(); it_1++){
			// 	cout<<"\t"<<(*it_1)->getObj(0)<<" "<<(*it_1)->getObj(1)<< " -- CD = "<<  (*it_1)->distance <<endl;
			// }
			// cout<<"ideal = "<<ideal[i][0]<<" "<<ideal[i][1]<<endl;
		...
		// int cont=0;
		// list<SolucaoEdgeSet *>::iterator ittt = subespacos[0].begin();
		// ittt++;
		// ittt++;
		// ittt++;
		// ittt++;
		// ittt++;
		// PR(NULL, *ittt, *subespacos[1].begin(), cont);
		...
		// 	cout<<"Subproblema "<<i<<" ("<< lambda[i][0] <<" "<<lambda[i][1]<< ")"<<endl; //( "<< bestSolution[i]->getObj(0)<<" "<< bestSolution[i]->getObj(1)<<"): "<<endl;
		// 	for (list<SolucaoEdgeSet *>::iterator it_1=subespacos[i].begin(); it_1!=subespacos[i].end(); it_1++){
		// 		cout<<"\t"<<(*it_1)->getObj(0)<<" "<<(*it_1)->getObj(1)<<endl;
		// 	}
		// 	cout<<"ideal = "<<ideal[i][0]<<" "<<ideal[i][1]<<endl;
