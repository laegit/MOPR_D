...
public:
	...
	void executar(SolucaoEdgeSet &s, double lambda[NUMOBJETIVOS], double alfa, int &contAval) {
		...
		uf.clear();
		for(i,nArestas)
			arestas[i].c = lambda[0]*f(0,arestas[i].a,arestas[i].b) + lambda[1]*f(1,arestas[i].a,arestas[i].b) + lambda[2]*f(2,arestas[i].a,arestas[i].b) + lambda[3]*f(3,arestas[i].a,arestas[i].b);
		...
		while (cont < NUMEROVERTICES-1) {
			...
			s.setObj(3,s.getObj(3)+f(3,s.edges[cont][0],s.edges[cont][1]));
