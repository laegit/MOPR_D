...
class SolucaoEdgeSet : public Solucao {
	...
	// // Copy constructor
	//   explicit Solution(const SolucaoEdgeSet & rhs)
	//   : assignment_(rhs.assignment_), objetivos_(rhs.objetivos_), distance(rhs.distance), getObj_NORMALIZADO(rhs.getObj_NORMALIZADO)
	//   {;} // ctor
	...
	void substAresta (int ai, int index, fitVecNode * fitVec[], int &contAval) {
		...
		setObj(3,getObj(3)+f(3,edges[ai][0],edges[ai][1]));
	...
	void printSolucao(FILE *f) {
		fprintf(f,"Custos = (%.3lf,%.3lf,%.3lf,%.3lf)\n",this->f[0],this->f[1], this->f[2], this->f[3]);
		fprintf(f,"Arestas = \n");
		for (int i=0;i<NUMEROVERTICES-1;i++)
			fprintf(f,"(%d - %d)\n",edges[i][0],edges[i][1]);
		fprintf(f,"\n");
	...
	void printPonto(FILE *f) {
		fprintf(f,"%.6lf %.6lf %.6lf %.6lf\n",getObj(0),getObj(1), getObj(2), getObj(3));
	}
	...
	bool confereObjetivos() {
		...
		for (int i=0;i<NUMEROVERTICES-1;i++) {
			...
			v[3] += f(3,edges[i][0],edges[i][1]);
		}
		if (fabs(v[0])-EPS > f[0] || fabs(v[1])-EPS > f[1] || fabs(v[2])-EPS > f[2] || fabs(v[3])-EPS > f[3]) return false;
		return true;
	}
