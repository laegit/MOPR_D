...
class ParetoSet {
	...
	void printSet(FILE *f) {
		...
		fprintf(f,"Ranges = [%.2lf,%.2lf], [%.2lf,%.2lf], [%.2lf,%.2lf], [%.2lf,%.2lf]\n",rangeAtual[0].min,rangeAtual[0].max,rangeAtual[1].min,rangeAtual[1].max, rangeAtual[2].min,rangeAtual[2].max,rangeAtual[3].min,rangeAtual[3].max);
		while (i != sol.end()) {
			s = *i;
			fprintf(f,"(%.10lf,%.10lf,%.10lf,%.10lf) -> %d\n",s->getObj(0),s->getObj(1),s->getObj(2),s->getObj(3),calcularGridPos(*s));
			i++;
		}
	}
	void printSetPoints(FILE *f) {
		...
		while (i != sol.end()) {
			s = *i;
			fprintf(f,"%.10lf %.10lf %.10lf %.10lf\n",s->getObj(0),s->getObj(1), s->getObj(2),s->getObj(3));
			i++;
		}
	}
