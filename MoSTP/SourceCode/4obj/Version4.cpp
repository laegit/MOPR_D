...
This code implements the fourth PathRelinking version for Bi-objective spanning tree 
		...
		*aux = *s1; // O(n)
		...
		for (int cont = 0; cont<totalSai && contAval<NUM_AVALIACOES; cont++){
				...
				if (amostralSai[iii]!=-1){
					...
					for (int jd=0; jd<totalEnt; jd++){
						...
						if (aux->uf.sameClass(a,b)==false){
							 ...
							 double newObj = newObj_0*lambda[0] +  newObj_1*lambda[1] + newObj_2*lambda[2]  + newObj_3*lambda[3];
							 if (newObj<mimObj){
							 	mimObj = newObj;
							 	entra = jd;
							 	sai = iii;
			...
			if (sai != -1 && entra != -1){
				...
				// cout<<"retorno->getSize()_v4 = "<<retorno->getSize()<<endl;
...
public:
	Version4(double lambda2[NUMOBJETIVOS]){
		memcpy(lambda,lambda2,sizeof(lambda));
