	...
	private:
		...
		*/
		...
		size_t find_ponto_referencia(vector<PontoReferencia> &pontos_referencia, vector<bool> &rp_isable){
			...
			for (size_t i = 0; i < pontos_referencia.size(); ++i)
				if (rp_isable[i]){ // NOVOOOO
					if(pontos_referencia[i].getQuantMembros() == min_size)
						min_pontos_referencia.push_back(i);
				}
			//cout<<"min_pontos_referencia.size() = "<<min_pontos_referencia.size()<<endl;
		...
		void atualizaPopulacaoNSGAII(SolucaoEdgeSet *novaPop[TAMANHOPOPULACAO_NSGAII]){
			...
			// for (list< SolucaoEdgeSet* >::iterator p = F[id_lastfront].begin(); p!=F[id_lastfront].end() && cont<TAMANHOPOPULACAO_NSGAII; p++){
			// 	*populacao[cont++] = **p;
			// }


			// crownding_distance_assigment(F[id_lastfront]);
			// F[id_lastfront].sort(compare2); // ordena por CD
			// for (list< SolucaoEdgeSet* >::iterator p = F[id_lastfront].begin(); p!=F[id_lastfront].end() && cont<TAMANHOPOPULACAO_NSGAII; p++){
			// 	*populacao[cont++] = **p;
			// }
		}
