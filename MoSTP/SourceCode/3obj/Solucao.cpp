...
class Solucao {
	...
	bool operator>> (Solucao &d) { // retorna true se f domina d.f
		return (f[0]<=d.f[0] && f[1]<=d.f[1] && f[2]<=d.f[2] && (f[0]<d.f[0] || f[1]<d.f[1] || f[2]<d.f[2]));
	}
	...
	bool operator==(Solucao &s) {
		if (fabs(s.getObj(0)-getObj(0)) < EPS && fabs(s.getObj(1)-getObj(1)) < EPS && fabs(s.getObj(2)-getObj(2)) < EPS)
			return true;
		return false;
	}
};
