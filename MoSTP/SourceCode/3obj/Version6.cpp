...
private:
	...
	bool A_domina_B(double a_obj1, double a_obj2, double a_obj3, double b_obj1, double b_obj2, double b_obj3){
		return (a_obj1<=b_obj1 && a_obj2<=b_obj2 && a_obj3<=b_obj3 && (a_obj1<b_obj1 || a_obj2<b_obj2 || a_obj3<b_obj3)); //minimizaçao
	}

	bool addNonDominandedSolution(double obj1, double obj2, double obj3, int sai,  int entra){
		...
		while (i != nonDominadedSolutions.end()) {
			// if (*nova >> *((*i).sol))
			if (A_domina_B(obj1,obj2, obj3, (*i).obj1, (*i).obj2, (*i).obj3))
				remover.push_back(i);
			// if (*((*i).sol) >> *nova)
			if (A_domina_B((*i).obj1, (*i).obj2, (*i).obj3, obj1,obj2, obj3) || ((*i).obj1==obj1 && (*i).obj2==obj2 && (*i).obj3==obj3))
				return false;
			i++;
		}
		...
		nonDominadedSolutions.push_front(auxSoluacao {obj1, obj2, obj3, sai, entra});
		return true;
	}
					...
					for (int jd=0; jd<totalEnt; jd++){
						...
						if (aux->uf.sameClass(a,b)==false){
							 ...
							 double newObj_2 = aux->getObj(2)-custos[2][origem][destino]+custos[2][a][b];
						 	 ...
						 	 addNonDominandedSolution(newObj_0,newObj_1,newObj_2, iii, jd);
