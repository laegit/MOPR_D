...
void getLowerAndUpperBound(){
  ...
  SolucaoEdgeSet s3 (NUMEROVERTICES-1);
	...
	for (int i=0;i<NUMEROVERTICES-1;i++) {
		for (int j=i+1;j<NUMEROVERTICES;j++) {
			arestas.push_back((auxEdgeStruct){i,j,custos[0][i][j],custos[1][i][j],custos[2][i][j],0});
		}
	} 
	int contlxo;
  ...
  //   s3.isTree();
	...
	cout<<"lower_bound = "<<s1.getObj(0)<<" "<<s2.getObj(1)<<" "<<s3.getObj(2)<<endl;
  ...
  //   s3.isTree();
	...
	cout<<"upper_bound = "<<s1.getObj(0)<<" "<<s2.getObj(1)<<" "<<s3.getObj(2)<<endl;
...
int main(int argc, char *argv[]){
 ...
 //    getPontoIdeal(ideal);
    ...
    calculaVetoresLambda_SISTEMATICO(subproblemas, 15); // NUMSUMPROBLEMAS-1);
     // begin coments
