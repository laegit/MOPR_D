	...
	private:
		...
		bool add_solution_subespaco(SolucaoEdgeSet *nova_solu, int subb){
			...
			while (i != subespacos[subb].end()) {
				...
				double distance_it = sqrt(((*i)->getObj(0)-ideal[subb][0])*((*i)->getObj(0)-ideal[subb][0]) + ((*i)->getObj(1)-ideal[subb][1])*((*i)->getObj(1)-ideal[subb][1])  +  ((*i)->getObj(2)-ideal[subb][2])*((*i)->getObj(2)-ideal[subb][2])   );
			...
			if (remover.size() == 0 && subespacos[subb].size()+1 > MAX_SUB_POPULATION) {
				...
				double distance_nova_solu = sqrt((nova_solu->getObj(0)-ideal[subb][0])*(nova_solu->getObj(0)-ideal[subb][0]) + (nova_solu->getObj(1)-ideal[subb][1])*(nova_solu->getObj(1)-ideal[subb][1]) +  (nova_solu->getObj(2)-ideal[subb][2])*(nova_solu->getObj(2)-ideal[subb][2]));
			...
			for (int cont = 0; cont<totalSai && contAval<NUM_AVALIACOES; cont++){
					...
					if (amostralSai[iii]!=-1){
						...
						for (int jd=0; jd<totalEnt; jd++){
							...
							if (aux->uf.sameClass(a,b)==false){
								...
								double distance = sqrt((new_objj_0-ideal[subproblemaATUAL][0])*(new_objj_0-ideal[subproblemaATUAL][0]) + (new_objj_1-ideal[subproblemaATUAL][1])*(new_objj_1-ideal[subproblemaATUAL][1])   +   (new_objj_2-ideal[subproblemaATUAL][2])*(new_objj_2-ideal[subproblemaATUAL][2]));
								// double distance = new_objj_0*lambda[subprol][0] + new_objj_1*lambda[subprol][1];
				...
				if (sai != -1 && entra != -1){
					...
					for (int subk=0; subk<NUMSUMPROBLEMAS; subk++){
						...
						double norma_lambda = lambda[subk][0]*lambda[subk][0] + lambda[subk][1]*lambda[subk][1]  + lambda[subk][2]*lambda[subk][2];
						double norma_aux = (aux->getObj(0)-Z_min[0])*(aux->getObj(0)-Z_min[0]) + (aux->getObj(1)-Z_min[1])*(aux->getObj(1)-Z_min[1])  +  (aux->getObj(2)-Z_min[2])*(aux->getObj(2)-Z_min[2]);; 
						double produto_escalar = (lambda[subk][0]*(aux->getObj(0) - Z_min[0]))   +  (lambda[subk][1]*(aux->getObj(1) - Z_min[1]))  +  (lambda[subk][2]*(aux->getObj(2) - Z_min[2]));
